...
function subject = EEGClockTask(sid,blk,varargin)
  ...
  strsec = num2str(GetSecs());
  % log all output of matlab
  diaryfile = fullfile( 'logs', [num2str(subject.subj_id) '_' strsec(1:5)  '_tcdiary.log'] );
  diary(diaryfile);
  ...
  try
     ...
     for emo=unique(subject.experiment{emotionC})'
         for facenum=unique(subject.experiment{facenumC})'
            stimfilename=strcat('C:\Users\STIMULUS\Documents\GitHub\CogEmoFaceReward\faces\',emo{1},'_',num2str(facenum),'.png');
     ...
     for i = startTrial:endTrial
        ...
        sendTrigger(COM4,trigger.(rew).(emo).face)
        ...
        sendTrigger(COM4,trigger.(rew).(emo).score);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function msgAndCloseEverything(message)
       sendTrigger(COM4,trigger.done) % send end code
       Priority(0); % set priority to normal 
       % send last message
   ...
   function scoreRxt(RT,func,varargin)
       ...
       [F_Mag F_Freq] = getScore(RT,func);
        ...
        %if(~isempty(varargin)), sendTrigger(COM4,varargin{1} + 4*inc>0),
        %end trigger can't be logical
