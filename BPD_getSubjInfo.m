...
function [order, runTotals, filename] = BPD_getSubjInfo(taskname)
...
% 2021-07-22 AndyP added redundancy check for subject ID
match = 0; 
while ~match
    subject.subj_id = NaN;
    ...
    sub1 = NaN;
    while isnan(sub1)
        sub1 = str2double(input('Please enter the subject ID number again:   ','s'));
        if isnan(sub1)
            fprintf('\n Subject id must be a number \n\n');
        end
    end
    if sub1==subject.subj_id
        match=1;
    else
        warning('Subject IDs did not match!');
    end
end
...
% %JW Ask which version of th elcokc task to run 8 block or 4 block
% if strcmpi(taskname, 'fMRIEmoClock')
%     subject.version = NaN;
%     while isnan(subject.version)
%         idInput = str2double(input('Enter the version to run 4 or 8 block: ','s')); %force to be numeric
%         if (idInput==4 || idInput==8)
%             subject.version = idInput;
%         else
%             fprintf('\n  Version must be either 4 or 8\n\n');
%         end
%     end
% end
...
%     if ( subject.version ==8)
%         csvfile='FaceFMRIOrder.csv'; %not checking for session at this time
%     else
%         csvfile='FaceFMRIOrder_4_blockTotal.csv'; %No wait inbetween blocks
%     end

    csvfile='FaceFMRIOrder_2_blockTotal.csv'; %2016: 3 blocks total
...
% 2021-07-22 AndyP, remove gender from prompts
subject.gender = []; 
...
if strcmpi(taskname, 'fMRIEmoClock')
    ...
    if subject.run_num==1 && ~ismember('runITI_indices', fields(subject))
        ...
        %             %For the time being this will work, really it should be
        %             %something along the lines of num_trials_total/length(locV.itimat)
        %             subject.runITI_indices = randsample(size(locV.itimat,1), 8);
        %         else
        %             subject.runITI_indices = randsample(size(locV.itimat,1), totalBlocks);
        %         end
        
        %SO rewrote this to just get the right number of ITI's here instead of
        %adding more later, reshape locV.itimat to have columns equal to the
        %number of trials.
        %locV.itimat = reshape(locV.itimat,[],(blockchangeidx(2)-blockchangeidx(1)));
        subject.runITI_indices = randsample(size(locV.itimat,1), totalBlocks);
