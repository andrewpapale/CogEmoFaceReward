...
elseif strcmpi(taskname, 'fMRIEmoClock')
    ...
    experiment=textscan(fid,'%d %d %s %s','HeaderLines',1,'Delimiter', ',');
    fclose(fid);
elseif strcmpi(taskname, 'fMRIEmoClockSupplement')
    ...
    elseif strcmpi(subject.task_ver ,'rev')
        fname = 'SuppleLookupTable.xlsx';
        if reversal_flag ==1
            csvfile='FaceBehavOrderREV_adaptive.csv';
        else
            csvfile='FaceBehavOrderSupple.csv';
        end
        contingency1 = 'DID';
        contingency2 = 'IDI';
    else
...
if strcmpi(taskname, 'fMRIEmoClock') || strcmpi(taskname, 'fMRIEmoClockSupplement')
